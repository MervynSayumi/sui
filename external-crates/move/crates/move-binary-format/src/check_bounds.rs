...
use crate::{
    ...
    file_format::{
        ...
        LocalIndex, ModuleHandle, Signature, SignatureToken, StructDefInstantiation,
        StructDefinition, StructFieldInformation, TableIndex, VariantJumpTable,
    },
...
impl<'a> BoundsChecker<'a> {
    ...
    fn check_data_type_handles(&self) -> PartialVMResult<()> {
        for struct_handle in self.view.data_type_handles() {
            self.check_data_type_handle(struct_handle)?
        }
        Ok(())
    }
    ...
    fn check_data_type_handle(&self, data_type_handle: &DataTypeHandle) -> PartialVMResult<()> {
        check_bounds_impl(self.view.module_handles(), data_type_handle.module)?;
        check_bounds_impl(self.view.identifiers(), data_type_handle.name)
    }
    ...
    fn check_struct_def(&self, struct_def: &StructDefinition) -> PartialVMResult<()> {
        check_bounds_impl(self.view.data_type_handles(), struct_def.struct_handle)?;
    ...
    fn check_enum_def(&self, enum_def: &EnumDefinition) -> PartialVMResult<()> {
        check_bounds_impl(self.view.data_type_handles(), enum_def.enum_handle)?;
        let type_param_count = self
            .view
            .data_type_handles()
        ...
        for variant_def in &enum_def.variants {
            check_bounds_impl(self.view.identifiers(), variant_def.variant_name)?;
            match self.view.enum_defs() {
                ...
                Some(enum_defs) => {
                    check_bounds_impl(enum_defs, variant_def.enum_def)?;
                }
            }
    ...
    fn check_code(
        ...
        for (bytecode_offset, bytecode) in code_unit.code.iter().enumerate() {
            ...
            match bytecode {
                ...
                | Abort | Nop => (),
                PackVariant(edi, vt)
                | UnpackVariant(edi, vt)
                | UnpackVariantImmRef(edi, vt)
                | UnpackVariantMutRef(edi, vt) => {
                    self.check_code_unit_bounds_impl_opt(
                        &self.view.enum_defs(),
                        *edi,
                        bytecode_offset,
                    )?;
                    self.view.enum_defs().map_or(Ok(()), |enum_defs| {
                        let (is_index_valid, variant_count) = enum_defs
                            .get(edi.into_index())
                            .map(|enum_def| {
                                (
                                    (*vt as usize) < enum_def.variants.len(),
                ...
                PackVariantGeneric(edii, vt)
                | UnpackVariantGeneric(edii, vt)
                | UnpackVariantGenericImmRef(edii, vt)
                | UnpackVariantGenericMutRef(edii, vt) => {
                    self.check_code_unit_bounds_impl_opt(
                        &self.view.enum_instantiations(),
                        *edii,
                        bytecode_offset,
                    )?;
                    if let Some(enum_inst) = self
                        .view
                        .enum_instantiations()
                        .and_then(|s| s.get(edii.into_index()))
                    {
                        // Check that the variant index is in bounds
                        self.view.enum_defs().map_or(Ok(()), |enum_defs| {
                            let (is_index_valid, variant_count) = enum_defs
                                .get(enum_inst.def.into_index())
                                .map(|enum_def| {
                                    (
                                        (*vt as usize) < enum_def.variants.len(),
                                        enum_def.variants.len(),
                        ...
                        let Some(sig) = self
                            .view
                            .signatures()
                            .get(enum_inst.type_parameters.into_index())
                        else {
                            return Err(self.offset_out_of_bounds(
                                StatusCode::INDEX_OUT_OF_BOUNDS,
                                IndexKind::Signature,
                                enum_inst.type_parameters.into_index() as usize,
    ...
    fn check_type(&self, ty: &SignatureToken) -> PartialVMResult<()> {
        ...
        for ty in ty.preorder_traversal() {
            match ty {
                ...
                DataType(idx) => {
                    check_bounds_impl(self.view.data_type_handles(), *idx)?;
                    if let Some(sh) = self.view.data_type_handles().get(idx.into_index()) {
