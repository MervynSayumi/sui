...
fn load_data_type_handle_index(
    cursor: &mut VersionedCursor,
...
trait CommonTables {
    ...
    fn get_data_type_handles(&mut self) -> &mut Vec<DataTypeHandle>;
    ...
    for table in tables {
        match table.kind {
            ...
            TableType::DECLARED_TYPE_HANDLES => {
                load_data_type_handles(binary, table, common.get_data_type_handles())?;
            }
...
fn load_data_type_handles(
    binary: &VersionedBinary,
    table: &Table,
    data_type_handles: &mut Vec<DataTypeHandle>,
) -> BinaryLoaderResult<()> {
    let start = table.offset as usize;
    let end = start + table.count as usize;
    let mut cursor = binary.new_cursor(start, end);
    while cursor.position() < table.count as u64 {
        ...
        data_type_handles.push(DataTypeHandle {
            module,
            name,
            abilities,
            type_parameters,
    ...
    let mut read_next = || {
        if let Ok(byte) = cursor.read_u8() {
            ...
            Ok(match S::from_u8(byte)? {
                ...
                S::DECLARED_TYPE => {
                    let sh_idx = load_data_type_handle_index(cursor)?;
...
fn load_struct_defs(
    ...
    let mut cursor = binary.new_cursor(start, end);
    while cursor.position() < u64::from(table.count) {
        let data_type_handle = load_data_type_handle_index(&mut cursor)?;
        ...
        struct_defs.push(StructDefinition {
            struct_handle: data_type_handle,
            field_information,
        });
    }
    Ok(())
...
fn load_enum_defs(
    ...
    let mut cursor = binary.new_cursor(start, end);
    while cursor.position() < u64::from(table.count) {
        let enum_handle = load_data_type_handle_index(&mut cursor)?;
        let field_information_flag = match cursor.read_u8() {
